    ...
    <ul class="list">
        {#each players as player}
            <li class="list-row items-center">
                <User />
                {player.name}
                <button class="btn btn-square btn-sm btn-error btn-soft" on:click={() => removePlayer(player.name)}>
                    <X />
                </button>
            </li>
        {/each}
    ...
    <div class="modal-box p-8">
        ...
        <div class="py-8 flex flex-col gap-4">
            <label class="floating-label">
                <span>Name *</span>
                <input type="text" placeholder="Name*" class="input input-lg w-full" bind:value={newPlayerName} />
            </label>
            <label class="floating-label">
                <span>Blacklist roles</span>
                <input type="text" placeholder="Blacklist roles" class="input input-lg w-full" bind:value={newPlayerBlacklist} />
            </label>
        </div>
